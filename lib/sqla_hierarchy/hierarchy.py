...
class Hierarchy(Executable, ClauseElement):
    ...
    def __init__(self, Session, table, select, **kw):
        ...
        if not hasattr(self, 'return_leaf') or not self.return_leaf == True:
            self.return_leaf = False
...
@compiles(Hierarchy, 'oracle')                                                      # GregM: Not inverted!
def visit_hierarchy(element, compiler, **kw):
...
@compiles(Hierarchy, 'postgresql')                                                  # GregM: Inverted to walk up the tree
def visit_hierarchy(element, compiler, **kw):
    ...
    if compiler.dialect.server_version_info < supported_db['postgresql']:
        ...
        sel2.append_column(label('connect_path', 
                      func.array_append(rec.c.connect_path, 
                                        getattr(element.table.c, element.child))    # GregM: Didn't need inverting as path building stays the same
                                ))
        ...
        sel2.append_column(literal_column(
                "%s=ANY(connect_path)" % getattr(element.table.c, 
                                                 element.parent)).label('cycle'))   # GregM: Inverted to parent to walk up the tree
        sel2 = sel2.where(and_(
            getattr(element.table.c,element.child)==getattr(rec.c,                  # GregM: Inverted to child to walk up the tree
                                                             element.parent),       # GregM: Inverted to parent to walk up the tree
            "not cycle"))
