...
(function($) {  
    ...
    $.fn.autoComplete = function(o) {
    	...
    	// MG2: added showAnimEasing and hideAnimEasing options
    	o = jQuery.extend({
		    ...
		    hideAnimProperties: {height:'0'}, // hide animation options, see http://api.jquery.com/animate/
    ...
    $.autoComplete.prototype = {
        init : function() {
		    ...
		    // MG2: renamed from wasVisible to lastHeight
		    this.lastHeight = 0;
		    ...
		    // MG2: hide the list when user clicked outside the list
		    ...
		    // MG2: prevent hiding of the list when click was inside the input field
			...
			this.field.keyup(function(e){
				...
				if(key != self.KEY_RETURN && key != self.KEY_TAB && key != self.KEY_ESC && key != self.KEY_UP && key != self.KEY_DOWN && key != self.KEY_PAGEDOWN && key != self.KEY_PAGEUP) {
					// MG2: prevent timeout while typing (using backspace the list would sometimes disappear)
					self.resetTimeout();
        ...
        getSuggestions : function(val){
        	...
        	// MG2: modified to lastHeight for animations
        	this.lastHeight = $('#'+this.acID).height();
        	if(this.lastHeight == null) {
        		this.lastHeight = 0;
        	}
        	
        	// MG2: removed the delete of the old list here (moved to AJAX call and is done in createList anyway),
        	//      done this to prevent hiding of the list all the time an AJAX request was made
        	
        	this.sInp = val;
        	
        	// MG: the last input has to be checked for the minimum length, not the total input (otherwise second input would show the list too with no input)
        	if(lastInput.length < this.options.minChars)
        	{
        		this.aSug = [];
        		this.nInpC = val.length;
        		// MG2: moved the remove of the old list to here, and modified it to clearSuggestions to have it animated
        		self.clearSuggestions();
        		return false;	
        	}
        	var ol = this.nInpC; 
        ...
        doAjaxRequest: function(input) {
        	// MG2: tidied code in this function a bit
        	...
        	// check AJAX url and input values (MG: also trim the last input and check if it's empty)
        	if(!url || $.trim(input) == '' || $.trim(this.sInp) == '') {
        		// MG2: moved the remove of the old list to here, and modified it to clearSuggestions to have it animated
        		self.clearSuggestions();
				return false;
        	} else if(input != this.field.val()) {
        		// MG2: to prevent hiding of the list even more, don't hide the list when input is different, the list will be updated by getSuggestions anyway after this function returns
        		return false;
        	}
			...
			var options = {
				...
				{
					// MG2: remove the old list here (moved from getSuggestions)
					$('#'+this.acID).remove();
				...
				error: (typeof self.options.onAjaxError == 'function') ? function(status) {
					...
					// MG2: remove the old list here (moved from getSuggestions)
					$('#'+this.acID).remove();
					// MG2: disabled as mentioned by Carpes
        ...
        createList: function(arr)
        {
        	...
        	// MG2: removed animSpeed again because of the easing functions
			...
			// MG2: removed the ac_bar div, wasn't used anymore
			var header = $('<div></div>').addClass('ac_header');
			header.append(hcorner);
			// MG2: append 2 inner divs to the main div to have the background visible while animating
			div.append($('<div></div>').addClass(this.options.className + '_inner1').append($('<div></div>').addClass(this.options.className + '_inner2')));
			div.append(header);
		    ...
		    // MG2: set the animate properties for the height
		    var startHeight = this.options.showAnimProperties.height ? (this.lastHeight ? this.lastHeight : 0) : divHeight;
		    this.options.showAnimProperties.height = divHeight;
		    
		    // start the animate by setting the desired properties (0 when used to animate, otherwise the value it has to be)
		    // MG2: using easing functions now for animate
		    div.height(startHeight)
		       .addClass("ac_tmpClass").removeClass("ac_tmpClass") // force a repaint because of changing the height
		       .width(this.options.showAnimProperties.width && !this.lastHeight ? 0 : w)
		       .css('opacity', this.options.showAnimProperties.opacity && !this.lastHeight ? 0 : div.css('opacity'))
		       .css('display', divDisplay)
		       .animate(this.options.showAnimProperties, {easing: this.options.showAnimEasing, duration: this.options.showAnimSpeed});
		    ...
		    // MG: prevent hiding of the list when click was inside the list
        ...
        clearSuggestions: function(){
        	...
        	// MG2: using easing functions now for animate
        	$('#'+this.acID).animate(this.options.hideAnimProperties, {easing: this.options.hideAnimEasing, duration: this.options.hideAnimSpeed, complete: function(){$(this).remove();}});
        }
