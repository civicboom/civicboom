...
<%def name="ShareThis(*args, **kwargs)">
    ...
    <%def name="share_this_icon(service)">
        <span class="st_${service}_custom icon16 ${service} link_pointer" title="${_('Share This via ')}${service}"><span>${service}</span></span>
    </%def>
    
    % for service in ['email', 'facebook', 'twitter', 'linkedin']:
        ${share_this_icon(service)}
    % endfor
    <span class="st_sharethis_custom icon16 share link_pointer" title="ShareThis"
        % for k,v in kwargs.iteritems():
    ...
    <span class="st_sharethis_custom icon16 share" title="ShareThis"></span>
    ...
    <span id="share2" class="icon16 share" title="ShareThis"></span>
