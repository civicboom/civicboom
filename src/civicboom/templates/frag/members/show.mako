...
<%def name="body()">
    ...
    <div class="frag_left_col">
        ...
        % if self.member.get('description'):
          ...
          <div style="clear:left;" class="frag_list">
            <h2><span class="icon16 ${self.attr.icon_type}"><span>descrition</span><div style="display:inline-block;padding-left:19px">Description</div></span></h2>
    ...
    <div class="frag_right_col">
        ...
        <div style="float: right; width: 66%; text-align: center;">
          ...
          <a style="text-align:left; float:left;" class   = "icon40 message"
          ...
          <a style="text-align:left; float:right;" class   = "icon40 notification"
...
<%def name="actions_specific()">
    ...
    ##    % if c.logged_in_persona and c.logged_in_persona.is_following(member['username']):
        ##${h.secure_link(url('member_action', action='unfollow', id=member['username'], format='redirect'), _(' '), title=_("Stop following %s" % member['username']), css_class="follow_action icon16 unfollow")}
        ##% else:  
        ##% endif
    ##% endif
    ...
    % if 'follow' in self.actions:
        ${h.secure_link(
            h.args_to_tuple('member_action', action='follow'    , id=self.id, format='redirect') ,
            value           = _('Follow') ,
            value_formatted = h.literal("<span class='icon16 follow'></span>%s") % _('Follow'),
            title           = _("Follow %s" % self.name) ,
            json_form_complete_actions = "cb_frag_reload('members/%s');" % self.id ,
        )}
        <span class="separtor"></span>
    ...
    % if 'unfollow' in self.actions:
        ${h.secure_link(
            h.args_to_tuple('member_action', action='unfollow'  , id=self.id, format='redirect') ,
            value           = _('Unfollow') ,
            value_formatted = h.literal("<span class='icon16 unfollow'></span>%s") % _('Stop Following'),
            title           = _("Stop following %s" % self.name) ,
            json_form_complete_actions = "cb_frag_reload('members/%s');" % self.id ,
        )}
        <span class="separtor"></span>
    ...
    % if 'join' in self.actions:
        ${h.secure_link(
            ...
            value           = _('Join _group') ,
            value_formatted = h.literal("<span class='icon16 join'></span>%s") % _('Join Group'),
            json_form_complete_actions = "cb_frag_reload('members/%s');" % self.id ,
        )}
        <span class="separtor"></span>
    % endif
    ...
    % if 'invite' in self.actions: #and c.logged_in_persona and c.logged_in_persona.__type__=='group':
        ...
        ${h.secure_link(
            ...
            value           = _('Invite') ,
            value_formatted = h.literal("<span class='icon16 invite'></span>%s") % _('Invite') ,
            title           = invite_text , 
            json_form_complete_actions = "cb_frag_reload('members/%s');" % self.id ,
        )}
        <span class="separtor"></span>
...
<%def name="actions_common()">
    % if 'message' in self.actions:
        ${popup.link(
            ...
            text  = h.literal("<span class='icon16 message'></span>%s") % _('Send Message'),
        )}
        <span class="separtor"></span>
    % endif
    
    % if 'settings_group' in self.actions:
        <a href="${h.url('edit_group', id=self.id)}" title="${_('_group Settings').capitalize()}"><span class="icon16 group"></span>${_('_group Settings').capitalize()}</a>
        <span class="separtor"></span>
    % endif
    
    % if 'settings' in self.actions:
        <a href="${h.url('settings')}" title="${_('Settings')}"><span class="icon16 settings"></span>${_('Settings')}</a>
        <span class="separtor"></span>
    % endif
    ...
    ${popup.link(
        ...
        title = _('Get widget'),
        text  = h.literal("<span class='icon16 widget'></span>%s") % _('Get widget'),
    )}
    <span class="separtor"></span>
...
<%doc>
        ...
        ##<a href='${url('formatted_member', id=d['member']['username'], format='rss')}' title='RSS for ${d['member']['username']}' class="icon16 rss"  ><span>RSS</span></a>
        <a href='${url.current(format='rss')}' title='RSS' class="icon16 rss"><span>RSS</span></a>
