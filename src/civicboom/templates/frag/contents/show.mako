    ...
    <div class="frag_right_col">
      <div class="frag_col">
        <div style="clear:left;" class="frag_list">
          <h2><span class="icon16 ${self.content['creator']['type']}"><span>${self.content['creator']['type']}</span><div style="display:inline-block;padding-left:19px; width: 100%">Created&nbsp;by</div></span></h2>
...
<%def name="content_details()">
    ...
    <div class="details">
        ...
        <%def name="format_date_if(title, date_input, fuzzy=True)">
          % if date_input:
            <tr>
              <td><span class="icon16 date" title="${title}"><span>&nbsp;</span></span></td>
              <td>${title}</td>
              % if fuzzy:
        ...
        <table class="content">
          <tr>
            <td><span class="icon16 boom"><span>Boom</span></span></td>
            <td>Booms</td>
            <td>${content['boom_count'] if 'boom_count' in content else '0'}</td>
            <td class="x"><span class="icon16 seen"><span>Views</span></span></td>
            <td>Views</td>
        ...
        <div class="iconholder padded">
          ${iconify('private', 'Private Content', 'icon16 private')}
          ${iconify('closed', 'Closed', 'icon16 closed')}
          % if content.get('approval')=='approved':
            <div class="icon16 approved" title="approved"><span>Approved</span></div>${_("Approved")}
          % endif
          <span style="float: right;">${iconify('edit_lock', 'Locked for editing', 'icon16 edit_lock')}</span>
        </div>
    </div>
        ...
        <%def name="format_date_if(title, date_input, fuzzy=True, trclass=None)">
          % if date_input:
            <tr${' class=%s' % trclass if trclass else ''}>
              <td><span class="icon16 date"><span>&nbsp;</span></span></td>
              <td>${title}</td>
              % if fuzzy:
...
<div style="padding-top: 20px;" class="acceptrequest">
  ...
  % if 'accept' in self.actions:
      ...
      ##${h.secure_link(h.args_to_tuple('content_action', action='accept'  , format='redirect', id=id), value=_('Accept'),  css_class="icon16 accept")}
      <span class="separtor"></span>
  % endif
...
<div class="comments">
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                ${popup.link(h.args_to_tuple('content_action', action='flag', id=comment['id']), title=_('Flag as') , class_='icon16 flag')}
            </td>
        </tr>
        % endfor
        <tr>
...
<%def name="actions_specific()">
    ...
    % if 'publish' in self.actions:
        ${h.secure_link(
            ...
            method = "PUT" ,
            value           = _('Publish') ,
            value_formatted = h.literal("<span class='icon16 publish'></span>%s") % _('Publish') ,
            json_form_complete_actions = "cb_frag_reload(current_element); cb_frag_reload('profile');" ,
    ...
    % if self.content['type'] != 'draft':
        ${h.secure_link(
            h.args_to_tuple('new_content', parent_id=self.id) ,
            value           = _("Respond") ,
            value_formatted = h.literal("<span class='icon16 respond'></span>%s") % _('Respond') ,
            json_form_complete_actions = h.literal(""" cb_frag(current_element, '/contents/'+data.data.id+'/edit.frag'); """)  , 
        )}
        ## AllanC the cb_frag creates a new fragment, data is the return fron the JSON call to the 'new_content' method
        ##        it has to be done in javascript as a string as this is handled by the client side when the request complete successfully.
    ...
    % if 'accept' in self.actions:
        ${h.secure_link(
            h.args_to_tuple('content_action', action='accept'  , format='redirect', id=self.id) ,
            value           = _('Accept') ,
            value_formatted = h.literal("<span class='icon16 accept'></span>%s") % _('Accept') ,
            json_form_complete_actions = "cb_frag_reload(current_element); cb_frag_reload('profile');" ,
        )}
        ##${h.secure_link(h.args_to_tuple('content_action', action='accept'  , format='redirect', id=id), value=_('Accept'),  css_class="icon16 accept")}
        <span class="separtor"></span>
    ...
    % if 'withdraw' in self.actions:
        ${h.secure_link(
            h.args_to_tuple('content_action', action='withdraw', format='redirect', id=self.id) ,
            value           = _('Withdraw') ,
            value_formatted = h.literal("<span class='icon16 withdraw'></span>%s") % _('Withdraw'),
            json_form_complete_actions = "cb_frag_reload(current_element); cb_frag_reload('profile');" ,
        )}
        <span class="separtor"></span>
    % endif
    ...
    % if 'boom' in self.actions:
        ${h.secure_link(
            ...
            value           = _('Boom') ,
            value_formatted = h.literal("<span class='icon16 boom'></span>%s") % _('Boom') ,
            json_form_complete_actions = "cb_frag_reload(current_element); cb_frag_reload('profile');" ,
        )}
        <span class="separtor"></span>
    % endif
    ...
    % if 'approve' in self.actions:
        ${h.secure_link(
            ...
            value_formatted = h.literal("<span class='icon16 approve'></span>%s") % _('Approve & Lock'),
    ...
    % if 'seen' in self.actions:
        ${h.secure_link(
            ...
            value           = _('Viewed') ,
            value_formatted = h.literal("<span class='icon16 seen'></span>%s") % _('Viewed'),
    ...
    % if 'dissasociate' in self.actions:
        ${h.secure_link(
            ...
            value_formatted = h.literal("<span class='icon16 dissasociate'></span>%s") % _('Disasociate'),
...
<%def name="actions_common()">
    ...
    % if 'edit' in self.actions:
        ...
        ><span class="icon16 edit"></span>${_("Edit")}</a>
        <span class="separtor"></span>
        ...
        ${h.secure_link(
            ...
            method = "DELETE",
            value           = _("Delete"),
            value_formatted = h.literal("<span class='icon16 delete'></span>%s") % _('Delete'),
    ...
    % if 'aggregate' in self.actions:
        ##${share.janrain_social(self.content, 'janrain', class_='icon16 share')}
    % endif
    
    % if 'flag' in self.actions:
        <a href='' onclick="$('#flag_content').modal(); return false;" title='${_("Flag inappropriate content")}'><span class="icon16 flag"></span>Flag</a>
        <span class="separtor"></span>
    % endif
