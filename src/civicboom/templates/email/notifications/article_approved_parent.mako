...
<%def name="body()">
    <%
        ...
        content = get_content(kwargs['extra_vars']['content_id'])
        assert content
        formatted_links = h.objs_to_linked_formatted_dict(article=content, parent=content.parent, member=content.creator)
    %>
    ...
    <h1>${_('Your _article request')}</h1>
        ...
        <h2>${_('Requested _article')}</h2>
            ...
            <h3>${'%(article)s' % formatted_links |n}</h3>
                ...
                <ul>
                  %for original_url in [media.original_url for media in content.attachments]:
