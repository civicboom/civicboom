...
${_('Civicboom lets people that need news stories get it from people that have it.')}</p>

<p>${_('How does it work?')}
${_('Civicboom works on two levels:')}</p>

<p>${_('1. Journalists, blogger, publishers, news organisations - get stories, videos, images and audio directly from your audience.')}
${_('2. People - you are the eyes and ears of a story and Civicboom is the tool that lets you share those stories directly with those that want and need them.')}</p>

<p>${_('What is the Boombox?')}
${_('A Boombox is a simple audience engagement "widget" that lives on a site through which people can directly post their stories and respond to requests for stories.')}</p>  

<p>${_('What can you do on the mobile app?')}
${_('You can respond to requests directly and share your news stories as they happen.')}</p>
...
<p>${_('What is a Hub?')}
${_('A Hub is a collection of registered users, unified under one "identity" - be it as an organisation, title or issue from which requests for stories can be created for others to reposed to. All Hubs can create a bespoke Boombox.')}</p>
...
${_('Observer: You can view drafts and comment on them, but not create or publish them.')}</p>

<p>${_('What happens when I switch Hubs')}
${_('All your actions-your requests, responses and content - will be posted under the name of that Hub.')}</p>
...
<p>${_('How do multiple users work on the same draft?')}
${_('Create a draft whilst under the persona of a Hub, then other members Hub members can edit that draft. Note: you must be under the persona of a Hub for this to work, it will not work with individual users for privacy reasons.')}</p>
...
${_('By embedding something you make it appear inside other webpages and blogs. For example, you might embed your request for news into site so that your readers can use it to give me their stories.')}</p>
