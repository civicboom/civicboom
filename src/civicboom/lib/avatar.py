...
def process_avatar(file_obj=None, url=None):
    ...
    with tempfile.NamedTemporaryFile(suffix=".png") as original:
        filename = None
        if url:
        ...
        with tempfile.NamedTemporaryFile(suffix=".png") as processed:
            ...
            # A Place In The Sun specifically request keeping their transparency
            #if im.mode == "RGBA":
            #    # AllanC - PNG's can have a 'background colour' set that the user is
            #    #          unaware of and the transparancy is silently converted to
            #    #          this colour by PIL create a new RGB image with a white
            #    #          background to paste avatar onto - this disposes of the
            #    #          avatars original backgourd colour
